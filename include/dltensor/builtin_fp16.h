...
template <typename SRC_T, typename SRC_REP_T, int SRC_SIG_BITS, typename DST_T,
          typename DST_REP_T, int DST_SIG_BITS>
static inline DST_T __truncXfYf2__(SRC_T a) {
  ...
  const SRC_REP_T roundMask =
      (SRC_REP_T(1) << (SRC_SIG_BITS - DST_SIG_BITS)) - 1;
  ...
  if (aAbs - underflow < aAbs - overflow) {
    ...
    absResult |= dstQNaN;
    absResult |=
        ((aAbs & srcNaNCode) >> (SRC_SIG_BITS - DST_SIG_BITS)) & dstNaNCode;
...
inline uint16_t f2h_ieee(float a) {
  return __truncXfYf2__<float, uint32_t, 23, uint16_t, uint16_t, 10>(a);
}

inline float h2f_ieee(uint16_t a) {
